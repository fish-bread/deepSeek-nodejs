...
const { createUser, searchUser, changeUser, deleteUser, changeUserHeadshot, searchUserBackground} = require('../service/user.service.js')
const {searchUserHistoryUid, updateUserHistory, createUserHistory } = require('../service/userHistory.service');
        ...
        if (ctx.request.body && search === undefined && user_captcha) {
            ...
            ctx.body = {
                ...
                user: {
                    ...
                    user_phone: newSearch.user_phone,
                    user_uid: newSearch.user_uid,
                    user_headshot: newSearch.user_headshot,
                    user_background: newSearch.user_background  
                },
                user_token: token,
            }
        }
...
router.post('/user_login_token', async (ctx,next) => {
    try {
        ...
        const decoded = ctx.state.user; // 通过 koa-jwt 解析后的 token 数据
...
router.post('/user/userHistory', async (ctx) => {
    const { user_uid, user_history } = ctx.request.body;
    const user = await searchUserHistoryUid(user_uid);
    await createUserHistory(user_history, user_uid)
    if (!user) {
        ctx.status = 200;
        console.log('用户历史为空')
        return
...
router.post('/user/searchUserHistory', async (ctx, next) => {
    const { user_uid } = ctx.request.body;
    const userHistory = await searchUserHistoryUid(user_uid);
    if (!userHistory) {
        ctx.status = 200;
        console.log('用户历史为空')
        return
    ...
    try {
        console.log('用户历史', userHistory.user_history);
        ctx.body = {
...
router.post('/user/updateUserHistory', async (ctx) => {
    const { user_uid, id } = ctx.request.body;
    const result = await updateUserHistory(id, user_uid);
    if (!result) {
        ctx.status = 400;
    } else {
        try {
