<script setup>
import {collapse, BodyLeft, BodyBack, isBack, newTitle} from '@/function/home/window.js'
import {nowSessionHistoryId, selectHistory, history} from "@/function/home/deepseekText.js";
const newToken = () => {
  newTitle.value = false
  nowSessionHistoryId.value = undefined
}
</script>

<template>
  <div ref="BodyLeft" class="body-left">
    <!--标题-->
    <div class="body-left-top">
      <div class="body-left-top-logo">
        <div style="font-size: 20px" title="what can i say">what can i say</div>
        </div>
      <div class="body-left-top-back"
           ref="BodyBack"
           @click="collapse"
           title="收起边框">
        <svg t="1741572231656" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1041" width="200" height="200"><path d="M329.728 493.285517a26.482759 26.482759 0 0 0 0 37.428966l287.126069 287.143724a26.482759 26.482759 0 0 0 37.464276-37.446621L385.906759 512 654.30069 243.588414a26.482759 26.482759 0 0 0-37.464276-37.446621L329.728 493.267862z" fill="#2c2c2c" p-id="1042">
        </path>
        </svg>
      </div>
    </div>
    <!--新对话-->
    <div class="body-left-new_token">
      <div class="body-left-new_token_button" @click="newToken">
    ...
    <!-- 历史信息 -->
    <div class="body-left-history" v-show="!isBack">
      <div
        class="body-left-history-button"
        v-for="(item, index) in history"
        :key="index"
        :title="item.history_id"
        :class="{ 'selected': nowSessionHistoryId === item.history_id }"
        @click="selectHistory(item.history_id)"
      >
        <div style="overflow: hidden">{{ item.history_id }}</div>
      </div>
    </div>
    <!--用户设置-->
    <div class="body-left-top-user">
      <div class="body-left-top-user-button">
        <div class="body-left-top-user-button-img">
          <img src="/src/assets/img/2.jpg" alt="" />
        </div>
        <div class="body-left-top-user-button-name">个人信息</div>
      </div>
    </div>
  </div>
</template>
...
.body-left-history-button:hover {
  background-color: #b3e5fc;
}
.selected {
  background-color: #81d4fa; /* 选中时的背景颜色 */
...
.body-left {
  ...
  display: grid;
  grid-template-rows: 90px 78px 600px auto;
}
.body-left-top {
  width: 100%;
  height: 90px;
...
.body-left-new_token_button:hover {
  background-color: #b3e5fc;
}
.body-left-history {
  height: 100%;
...
.body-left-top-user-button:hover {
  background-color: #b3e5fc;
}
.body-left-top-user-button-img {
  height: 30px;
}
img {
  width: 30px;
